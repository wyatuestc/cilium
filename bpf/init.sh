...
function setup_nat_box()
{
	...
	bpf_compile cilium-nat-in "$OPTS" "egress" bpf_nat_rev_out.c bpf_nat_rev_out.o to-netdev $CALLS_MAP
	...
	bpf_compile cilium-nat-out "$OPTS" "ingress" bpf_netdev.c bpf_netdev_nat_out.o from-netdev $CALLS_MAP
	...
	# time.
	ip rule list from all lookup local pref 100 || {
...
echo "#endif" >> $RUNDIR/globals/node_config.h
